...
class ProdutoRepositoryTest {
    ...
    @Test
    void findByEmpresaAndAtivoTrueShouldReturnOnlyActiveProductsFromEmpresa() {
        ...
        List<Produto> produtosAtivos = produtoRepository.findByEmpresaAndAtivoTrue(empresaA);

        assertThat(produtosAtivos)
                .extracting(Produto::getId)
                .containsExactlyInAnyOrder(arroz.getId(), feijao.getId());
    }

    @Test
    void findByEmpresaAndNomeContainingIgnoreCaseShouldSearchInsideEmpresaOnly() {
        ...
        Produto arrozIntegral = persistProduto(empresaA, categoriaA, "Arroz Integral", false, 7, 3);
        persistProduto(empresaB, categoriaB, "Arroz Tipo 1", true, 6, 2);
        entityManager.clear();

        List<Produto> encontrados = produtoRepository.findByEmpresaAndNomeContainingIgnoreCase(empresaA, "arroz");

        assertThat(encontrados)
                .extracting(Produto::getId)
                .containsExactlyInAnyOrder(arrozBranco.getId(), arrozIntegral.getId());
    }
