...
public class ProdutoService {
    ...
    public List<Produto> listarProdutosPorEmpresa(Long empresaId) {
        Empresa empresa = empresaService.buscarEmpresaPorId(empresaId);
        return produtoRepository.findByEmpresaAndAtivoTrue(empresa);
    }

    public Produto buscarProdutoPorId(Long empresaId, Long produtoId) {
        Empresa empresa = empresaService.buscarEmpresaPorId(empresaId);
        return produtoRepository.findById(produtoId)
                .filter(produto -> produto.getEmpresa().equals(empresa))
                .orElseThrow(() -> new ProdutoNotFoundException("Produto não encontrado para o ID: " + produtoId));
    }
